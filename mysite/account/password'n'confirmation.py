...
class ConfirmationBase(models.Model):
    ...
    class AlreadyConfirmed(Error):
        ...
        def create(cls, user, **kwargs):
            ...
            """
            token = hookset.generate_email_confirmation_token(user.email)
...
class PasswordResetConfirmation(ConfirmationBase):
    ...
    objects = PasswordResetConfirmationManager()

    def send_registration(self, **kwargs):
        """Send an email with a link for registration.
        ...
        if self.is_valid(self.token):
            ...
            hookset.send_invitation_email([self.user.email], ctx)
            # TODO: add signal
        else:
    ...
    def send_reset_link(self, **kwargs):
        ...
        if self.is_valid(self.token):
            ...
            hookset.send_password_reset_email([self.user.email], ctx)
            # TODO: add signal
        else:
            raise Exception('Invalid token.')
    ...
    def after_password_changed(self, **kwargs):
        ...
        hookset.send_password_change_email([self.user.email], ctx)
